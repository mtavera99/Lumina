...
export function Intelligence() {
  ...
  const disconnect = () => {
    ...
    setSession(null); setWorkspace(null); setServiceStatus(null); setQuery(''); setConversationId(undefined)
    ...
    <div className="intelligence">
      <section className="intelligence-hero card">
        ...
        <div className="intelligence-hero-action">
          {session ? <><div className="signed-user">{session.picture && <img src={session.picture} alt="" referrerPolicy="no-referrer" />}<div><b>{session.name}</b><span>{session.email}</span></div></div><div className="intelligence-actions"><button className="btn btn-primary" onClick={sync} disabled={loading || syncingContext}>{loading || syncingContext ? 'Sincronizando…' : '↻ Sincronizar'}</button><button className="btn btn-ghost" onClick={disconnect}>Desconectar</button></div></> : <button className="btn btn-primary" onClick={connect} disabled={loading || !googleWorkspaceConfig.configured}>{loading ? 'Conectando…' : 'Conectar mi cuenta de Google'}</button>}
        </div>
      </section>
      ...
      <div className="grid intelligence-connections">
        <ConnectionCard icon="✉️" name="Gmail" description="Filtra correos verificados de Read AI" state={session ? 'connected' : googleWorkspaceConfig.configured ? 'ready' : 'pending'} />
      ...
      {session && workspace ? <>
        <div className="grid intelligence-stats">
          <div className="card stat"><div className="stat-label">Reportes de Read AI</div><div className="stat-value">{workspace.reports.length}</div><div className="stat-sub">Ultimos 12 meses</div></div>
      ...
      <div className="intelligence-privacy"><span>🛡️</span><p><b>Privacidad:</b> Gmail se filtra para reportes de read.ai. El agente omite enlaces, codigos y credenciales de acceso a reuniones. Los secretos de Supabase y Gemini viven solamente en Netlify; el navegador nunca los recibe.</p></div>
    </div>
  )
}
